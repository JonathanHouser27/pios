...
void init_pfa_list(void) {
    for (int i = 0; i < NUM_PAGES; i++) {
        physical_page_array[i].physical_addr = (void *)(0x1000 * i); // Assign a physical address
        ...
        list_add(&free_physical_pages, &physical_page_array[i]); // Add to the free list using list_add
    }
}
